 ...
 *
 */
package cn.luliangwei.lib.utils.exception;
...
import cn.luliangwei.lib.utils.Validator;
...
public class ExceptionFormater {
    ...
    private static final String DEFAULT_TRACE_FORMAT_TEMPLATE = "%n|-- STACK TRACE  :"
                                                              + "%n|                ^";
    private static final String DEFAULT_TRACE_BASE_FORMAT_TEMPLATE  =  DEFAULT_TRACE_DATE_FORMAT_TEMPLATE
                                                                    +  DEFAULT_TRACE_FILE_NAME_FORMAT_TEMPLATE
                                                                    +  DEFAULT_TRACE_CLASS_NAME_FORMAT_TEMPLATE
                                                                    +  DEFAULT_TRACE_METHOD_NAME_FORMAT_TEMPLATE
                                                                    +  DEFAULT_TRACE_LINE_NUMBER_FORMAT_TEMPLATE
                                                                    + DEFAULT_TRACE_MESSAGE_FORMAT_TEMPLATE
                                                                    + DEFAULT_TRACE_FORMAT_TEMPLATE;
    ...
    private static String formatMessage(Throwable e) {
        return e.getClass().getName().concat(": ").concat(String.valueOf(e.getLocalizedMessage()));
    }
    ...
    private static String simplifyClassName(String className) {
        ...
        for (String pkgPath : pkgPaths) {
            sb.append(pkgPath.charAt(0)).append('.');
        }
        return sb.append(extractSimpleName).toString();
    }

    /**
     * 格式化异常消息.<br/>
     * 注：默认追踪的异常堆栈消息中包含的关键词: <code>cn.signit</code>
     * <br/>
     * 默认格式化后的消息形如：
     * 
     * <pre>
